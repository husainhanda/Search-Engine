	...
	public static void searchWords(File srcFile,int number,Matcher m,String pattern)
	{
		try
    	{
			...
			while ((line = rd.readLine()) != null){
				m.reset(line);
				while (m.find()) {
					SearchEngineApplication.val.add(m.group());
				}
			}
			rd.close();
			for(int n= 0; n<SearchEngineApplication.val.size(); n++){
				SearchEngineApplication.values.put(SearchEngineApplication.val.get(n), findEditDistance(pattern.toLowerCase(),SearchEngineApplication.val.get(n).toLowerCase()));
			}
    	}     
    	catch(Exception e)
    	{
